...
import { Bookmark, Camera, Leaf, Medal, Shield, Sparkles } from "lucide-react";
...
/**
 * "My places" (Screen 8, renamed per handover 1f) — honestly labelled: data is
 * tied to THIS device (a device-scoped token, not an account), the report list
 * is "stored on this device". Driven by real data: reports owned by the
 * device's author_token. States: with-data / empty.
 */
    ...
    <div className="flex h-screen flex-col">
      <div className="flex-1 overflow-auto">
        ...
        <div className="px-6 pb-5 pt-4 text-center" style={{ background: "radial-gradient(120% 80% at 50% 0%, var(--tint-soft), var(--surface))" }}>
          <div className="mx-auto mb-2.5 grid h-16 w-16 place-items-center rounded-[20px] bg-surface-card shadow-card">
            <DrosiaMark className="h-[34px] w-auto text-primary" />
          </div>
        ...
        {hasData ? (
          <div className="px-4">
            ...
            <div className="flex gap-2.5">
              <Badge icon={<Medal size={26} aria-hidden />} label={dict.impact.b1} locked={impact.reported < 1} />
              <Badge icon={<Shield size={26} aria-hidden />} label={dict.impact.b2} locked={impact.reported < 5} progress={Math.min(100, impact.reported * 20)} />
              <Badge icon={<Sparkles size={26} aria-hidden />} label={dict.impact.b3} locked={impact.resolved < 1} />
            </div>

            <h2 className="my-3 flex items-baseline gap-2 font-display text-[15px] font-black">
              {dict.impact.mine}
              <span className="text-[10px] font-bold text-muted">{dict.impact.mineDevice}</span>
            </h2>
            {impact.mine.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-1.5 font-display text-[13px] font-extrabold">
                      <CategoryIcon category={m.category} size={15} className="flex-none text-primary-ink" />
            ...
            <div className="mt-2 flex items-center gap-2.5 rounded-[14px] border border-[#F4D03F] bg-[#FFF8E6] p-3">
              <Bookmark size={18} className="flex-none text-[#7A5C0A]" aria-hidden />
              <div className="text-[11px] leading-relaxed text-[#7A5C0A]">{dict.impact.bookmark}</div>
            </div>
          </div>
        ) : (
          <div className="px-6 py-8 text-center">
            <Leaf size={46} className="mx-auto text-success" aria-hidden />
            ...
            <Link
              href="/report"
              className="inline-flex items-center gap-2 rounded-[14px] bg-primary px-6 py-3.5 font-display text-[15px] font-extrabold text-white"
            >
              <Camera size={17} aria-hidden /> {dict.impact.emptyCta}
            </Link>
          </div>
        )}
      </div>
...
function Badge({ icon, label, locked, progress }: { icon: React.ReactNode; label: string; locked?: boolean; progress?: number }) {
  return (
    <div
    ...
    >
      <div className="grid place-items-center" style={{ color: locked ? "var(--muted)" : "var(--primary-ink)" }}>
        {icon}
      </div>
